...
class BattleshipGame:
    ...
    def __init__(self, num_of_players):
        # Create the players.
        if num_of_players == 1:
            self.players = [HumanPlayer("Player 1"), ComputerPlayer()]
...
class HumanPlayer:
    ...
    def take_turn(self):
        ...
        while position is None:
            try:
                # Get attack history information.
                print("You have attacked " + str(self.hit+self.miss) + " times, with " + str(self.hit) + \
                      " hits and " + str(self.miss) + " misses.")
...
class ComputerPlayer:
    # Initialize the computer player with the name of "Computer player", a blank board,
    # and a fleet of five boats.
    def __init__(self):
        self.player_name = "Computer player"
        self.board = Board()
        self.fleet = [Boat("Aircraft Carrier", 5), Boat("Battleship", 4), Boat("Submarine", 3), Boat("Destroyer", 3), \
                      Boat("Patrol Boat", 2)]
        self.opponent = None
    ...
    # Link the computer player to human opponent.
    def set_opponent(self, opponent):
        self.opponent = opponent
    ...
    def position_fleet(self):
        ...
        for boat in self.fleet:
            self.position_boat(boat)
        print("The Computer Player fleet is ready to play.")

    # Position a boat.
    def position_boat(self, boat):

        # Get random choice from Vertical or Horizontal.
        ...
        while coords is None:
            try:
                ...
                coords = [random.randrange(10),random.randrange(10)]
                ...
                boat.set_position(x,y)
                if not self.board.add_boat(boat):
                    # The board refused to add the boat!!! Raise an exception so that the computer player
                    # is forced to try a new position.
                    raise Exception
            except:
                coords = None

    # This function manages a single turn for the computer player.
    def take_turn(self):
        ...
        while coords is None:
            try:
                # Get random attack position.
                coords = [random.randrange(10), random.randrange(10)]

                if coords in self.attack_list:
                    # If the position was attacked before, raise exception to force the computer player to get
                    # another random position.
                    raise Exception
                else:
                    x = int(coords[0])
                    y = int(coords[1])
                    # Append the position to attacked history list.
                    self.attack_list.append(coords)
            except:
                coords = None
        ...
        hit_flag = self.opponent.board.attack(x, y)
        if hit_flag:
            print("The Computer Player hit you a boat at position " + "(" + str(x) + ", " + str(y)+")!" )
        else:
            print("The Computer Player missed at position " + "(" + str(x) + ", " + str(y) + ").")
